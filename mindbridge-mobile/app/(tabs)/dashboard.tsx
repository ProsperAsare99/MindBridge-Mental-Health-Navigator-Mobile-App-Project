...
const WeeklyPulse = ({ theme, styles }: any) => {
  const days = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
  const scores = [0.8, 0.6, 0.9, 0.4, 0.7, 0.8, 0.5]; 
      ...
      <BlurView intensity={theme.isDark ? 30 : 60} tint={theme.isDark ? 'dark' : 'light'} style={styles.pulseGlass}>
        <View style={styles.pulseHeader}>
          <View>
            ...
            <Text style={[styles.pulseSubtitle, { color: theme.colors.text.secondary }]}>Mostly Calm & Steady</Text>
          </View>
...
const AppleWidget = ({ title, subtitle, icon: Icon, color, onPress, theme, styles, size = 'square', delay = 0 }: any) => {
    ...
    const checkStatus = async () => {
      try {
        ...
        setRituals({
          ...
          journal: res.data.recentJournal && res.data.recentJournal.some((log: any) => new Date(log.createdAt).toDateString() === todayStr),
  ...
  return (
    <View style={dashStyles.container}>
      ...
      <ScrollView contentContainerStyle={[dashStyles.scrollContent, { paddingTop: insets.top }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={dashStyles.section}><WeeklyPulse theme={theme} styles={dashStyles} /></View>
        ...
        <View style={dashStyles.section}><AppleWidget title="The Oracle" subtitle="AI Personal Guide" icon={Bot} color={theme.colors.plum} size="wide" delay={400} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/ai-guide')} /></View>

        <View style={dashStyles.sectionCompact}>
          <View style={dashStyles.sectionHeader}><Text style={[dashStyles.sectionTitleText, { color: theme.colors.text.primary }]}>Explore Tools</Text></View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={dashStyles.horizontalScroll} snapToInterval={154} decelerationRate="fast">
            <AppleWidget title="Assessments" subtitle="Daily check" icon={ClipboardList} color={theme.colors.accents.slate} size="fixed" delay={450} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/assessments')} />
            <AppleWidget title="Resources" subtitle="Library" icon={Library} color={theme.colors.accents.forestGreen} size="fixed" delay={500} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/resources')} />
            <AppleWidget title="Community" subtitle="Connect" icon={Users} color={theme.colors.accents.dustyRose} size="fixed" delay={550} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/community')} />
          </ScrollView>
        </View>
        ...
        <View style={dashStyles.section}>
          ...
          <View style={[dashStyles.listContainer, { backgroundColor: theme.colors.surface, borderColor: theme.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)' }]}>
            <AppleWidget title="Wellness Journey" subtitle="Your roadmap" icon={Clock} color={theme.colors.plum} size="list" delay={600} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/journey')} />
            <View style={[dashStyles.divider, { backgroundColor: theme.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }]} />
            <AppleWidget title="Crisis Support" subtitle="Help 24/7" icon={ShieldAlert} color={theme.colors.accents.terracotta} size="list" delay={700} theme={theme} styles={dashStyles} onPress={() => router.push('/(tabs)/crisis')} />
          </View>
        </View>
      </ScrollView>
    </View>
...
const dashStyles = StyleSheet.create({
  ...
  ritualsContainer: { marginHorizontal: 24, borderRadius: 32, padding: 24, marginBottom: 8, borderWidth: 1 },
